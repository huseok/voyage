...
阿里云单机部署：**`deploy/aliyun/README.md`**（手动命令速查）、**`deploy/aliyun/release.sh`**（一键发版）、**`deploy/aliyun/DEPLOY_STEP_BY_STEP.md`**（分步详解）。**生产域名 HTTPS**：**`deploy/aliyun/DOMAIN_SETUP.md`**（含 2026-05-17 上线实录）。远端库连接串请用环境变量注入。
